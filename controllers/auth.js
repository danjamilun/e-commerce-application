...
exports.signup = (req, res) => {
  ...
  user.save((err, user) => {
    //nakon sto se dobije novi user baziran na request body-u sprema se to u bazu
    ...
    res.json({
      //ako je normalno posalji respons odnosno user u json-u
      user,
    });
  });
};
exports.signin = (req, res) => {
  ...
  User.findOne({ email }, (err, user) => {
    //pronaci po email-u i zatim izvrsi arrow funkciju
    if (err || !user) {
      return res.status(400).json({
    ...
    if (!user.authenticate(password)) {
      //ako autentifikacija nije prosla u toj metodi definiranoj u models/user.js
      return res.status(401).json({
        //vracamo odgovor odnosno error
...
exports.isAdmin = (req, res, next) => {
  if (req.profile.role === 0) {
    //0 je standardni korisnik, 1 je admin
    return res.status(403).json({
      //user is not admin
